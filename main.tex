...
\begin{lstlisting}[language=BNFLike, gobble=2]
  ...
  TraitItemMethod ::=
    FnPrefix "fn" Identifier Generics? WhereClauses?
...
\begin{itemize}
  ...
  \item 関連定数は \verb|feature(associated_consts)| が提供するunstable機能です。
  \item 関連型は単相です。
  ...
  \item 現在の実装では、(メソッドの実装が提供されているか否かによらず)メソッドの引数名は省略できます。その影響で、メソッドの引数部に書けるパターンに構文上の制約が存在します。この構文はRFC~1685で廃止が決定されました。
\end{itemize}
